...
/** Pulls a name hint out of an OAuth provider's profile data (e.g. Google, Discord), if any. */
export async function fetchNameHint(): Promise<{ firstName: string; lastName: string } | null> {
  ...
  // Google uses given_name/family_name; Discord has no split name at all (often just a
  // single-word display name/username). Both also provide a combined name/full_name we
  // can fall back to splitting on whitespace.
